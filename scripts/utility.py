...
def generate_sprite(
    ...
    if (
        ...
        if age in ["kitten", "adolescent"]:
            cat_sprite = str(choice([32, 35]))
        else:
            if cat.pelt.length == "long":
                cat_sprite = str(choice([31, 34]))
            else:
                cat_sprite = str(choice([30, 33]))
    else:
    ...
    try:
        ...
        if cat.pelt.dark_patches is not None:
            ...
            if (
                ...
                in sprites.dark_patches_tints["tint_colours"]
            ):
                tint = pygame.Surface((sprites.size, sprites.size)).convert_alpha()
                tint.fill(
                    tuple(
                        sprites.dark_patches_tints["tint_colours"][
                            cat.pelt.dark_patches_tint
                        ]
                    )
                )
                dark_patches.blit(tint, (0, 0), special_flags=pygame.BLEND_RGB_MULT)
        ...
        if cat.pelt.dark_points:
            ...
            if (
                ...
                dark_points.blit(tint, (0, 0), special_flags=pygame.BLEND_RGB_MULT)
