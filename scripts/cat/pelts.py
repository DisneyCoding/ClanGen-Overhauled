...
class Pelt:
    ...
    def dark_patches_inheritance(self, parents: tuple):
        ...
        for p in parents:
            if p:
                ...
                if p.pelt.points:
                    par_dark_points.append(p.pelt.dark_points)
        ...
        # Direct inheritance. Will only work if at least one parent has white patches, otherwise continue on.
        if par_darkpatches and not random.randint(
            0, game.config["cat_generation"]["direct_inheritance"]
        ):
            # This ensures Torties and Calicos won't get direct inheritance of incorrect white patch types
            ...
            if _temp:
                ...
                # Direct inheritance also effect the point marking.
                if par_dark_points and self.name != "Tortie":
                    self.dark_points = choice(par_dark_points)
                else:
        ...
        chosen_dark_paches = choice(
            random.choices(dark_list, weights=weights, k=1)[0]
        )

        self.dark_patches = chosen_dark_paches
        if self.dark_points and self.dark_patches in (
            Pelt.high_dark,
            Pelt.mostly_dark,
            "FULLDARK",
        ):
    ...
    def init_tint(self):
        ...
        if self.dark_patches or self.dark_points:
            # Now for white patches
            base_tints = sprites.dark_patches_tints["possible_tints"]["basic"]
            if self.colour in sprites.cat_tints["colour_groups"]:
                color_group = sprites.dark_patches_tints["colour_groups"].get(
                    self.colour, "white"
                )
                color_tints = sprites.dark_patches_tints["possible_tints"][color_group]
            else:
                color_tints = []

            if base_tints or color_tints:
                self.dark_patches_tints = choice(base_tints + color_tints)
            else:
                self.dark_patches_tints = "none"
        else:
            self.dark_patches_tints = "none"
